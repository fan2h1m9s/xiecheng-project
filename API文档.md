...
- **请求参数**:
  ...
  | --- | --- | --- | --- |
  | id | number | 是 | 订单 ID |
  | orderStatus | number | 否 | 订单状态（1: 待支付, 2: 已支付, 3: 已完成, 4: 已取消） |
  | orderTotalPrice | number | 否 | 订单总价 |
  | orderCheckInTime | string | 否 | 入住时间（ISO 格式） |
  | orderCheckOutTime | string | 否 | 退房时间（ISO 格式） |
- **响应格式**:
  - **成功响应**:
    ```json
    {
